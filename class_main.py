...
from src.tools import load_cls_data, write_to_csv
from src.evaluate import Evaluator_cls

from modules.data_generator import data_generator, tokenizer
from modules.cls_model import model

from config.cls_config import train_parameters, to_h5_parameters, infer_parameters
...
test_data = load_cls_data(test_data_path)
train_data = load_cls_data(train_data_path)
...
def train():
    evaluator = Evaluator_cls(model, valid_generator, train_parameters.checkpoints_path)
    model.fit(
