...
class Train_parameters:
    ...
    batch = 32
    epoch = 2
    train_data_root = './data/1108_cls'

    # 保存权重
    checkpoints_root = './cls_checkpoints/bert_{}_{}'.format(layer, the_time)
...
class To_h5_parameters:
    ...
    load_model_time_2_h5 = '2023-11-14'

    load_checkpoints_root = './cls_checkpoints/bert_{}_{}'.format(model_p.layer, load_model_time_2_h5)
    load_checkpoints_path = os.path.join(load_checkpoints_root, model_p.save_model_name)
    # 转 h5 命名，要和训练保存的名字已知，避免混乱。
    save_h5_path = './cls_checkpoints/class-bert_{}.h5'.format(load_model_time_2_h5)
...
class Infer_parameters():
    '''
        加载的模型路径。
    '''
    modul_time = '2023-11-14'
    h5_path = './cls_checkpoints/class-bert_{}.h5'.format(modul_time)
